...
class spAcl
{
	...
	public function def_prompt()
	{
		...
		echo "<html><head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\"><script>function sptips(){alert(\"该页面您无权访问\");location.href=\"{$url}\";}</script></head><body onload=\"sptips()\"></body></html>";
		exit;
	}
...
class spAclModel extends spModel
{
	...
	public function check($acl_name = SPANONYMOUS, $controller, $action)
	{
		...
		if( $acl = $this->find($rows) ){
			$acl_name_arr = explode('|', $acl['acl_name']);
			foreach($acl_name_arr as $v){
				if($v == SPANONYMOUS || $v == $acl_name)return 1;
			}
			return 0;
		}else{
			return -1;
