...
class spController { 
	...
	{	
		...
		$this->spConfig = $GLOBALS['spConfig'];
	}
			...
			if( $this->has($name) ){
				if( null != $method ){
					switch (strtolower($method)) {
						case 'get':
							return $this->escape($_GET[$name]);
						case 'post':
							return $this->escape($_POST[$name]);
						case 'cookie':
							return $this->escape($_COOKIE[$name]);
					}
				}
